...
interface StoreBacklog {
    ...
    removeTaskBacklog: (id: number) => void
}
...
export const useStoreBacklog = create<StoreBacklog>((set) => ({
    ...
    addTaskBacklog: (task) => set((state) => ({ backlog: { ...state.backlog, task } })),
    editTaskBacklog: (newTask) => set((state) => ({ backlog: state.backlog.map(task => task.id === newTask.id ? newTask : task) })),
    removeTaskBacklog: (id) => set((state) => ({ backlog: state.backlog.filter(task => task.id !== id) }))
}))
